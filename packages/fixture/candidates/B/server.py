#!/usr/bin/env python3
"""Behavior-compatible refund and fee service, candidate B."""
...
class RefundApplication:
    ...
    def run_diagnostic(self, payload: Any) -> Tuple[int, Dict[str, str]]:
        if not isinstance(payload, dict):
            raise RequestError("JSON body must be an object")
        target = required_text(payload, "target", 200)
        ...
        return 200, {"output": output, "target": target}
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        try:
            ...
            elif route == "/admin/diagnostics":
                status, body = self.application.run_diagnostic(payload)
            else:
                self.send_error_body(404, "not_found", "route not found")
                return
...
def main() -> None:
    ...
    print(f"candidate B listening on {args.bind}:{args.port}")
    try:
